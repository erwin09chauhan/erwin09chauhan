...
- **[Kiwi Careers Tracker](https://github.com/erwin09chauhan/kiwiCareersTracker)** ([live demo](https://kiwicareerstracker.pages.dev/)) — a job application tracker that follows each application from applied through to offer or rejection. Every status change fans out through a message queue to independent consumers, one writing the audit log, another pushing a live notification, so the pieces stay decoupled. There's a background service that watches for due reminders and Redis handling caching and rate limiting. Built on Clean Architecture with CQRS.
